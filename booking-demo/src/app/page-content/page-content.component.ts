...
export class PageContentComponent implements OnInit {
  ...
  verifyCode = false;
  phoneNumber: string = "";
  pinApi: PinApi = new PinApi("", "", "", new Customer(0, "", "", 0));
  onSubmitBook() {
    this.submmited = true;
    this.stylistService.checkPhoneOfCustomer(+this.phoneNumber).subscribe(
      pinApi => { this.pinApi = pinApi; console.log(`pin api: ${JSON.stringify(this.pinApi)}`); }
    );
  ...
  //change timeline to array status
  changeTimeLineToStatus():void {
